...
function Description(props) {
    ...
    const generateDesc = () => {
        ...
        if (Number(props.data.purchased) > 0) {
            desc = 'Dikonfirmasi ' + props.data.purchased;
        } else {
            if (Number(props.data.canceled) > 0) {
                desc = 'Dibatalkan ' + props.data.canceled;
            } else {
                if (Number(props.data.updated) > 0) {
                    desc = 'Diperbarui ' + props.data.updated;
                } else {
    ...
    return (
        <div>
            <div className="mb-3 mt-2">
                ...
                <p className="color-gray-dark-v4">
                        ...
                        <header className="bg-light p-3">
                            <Row className="font-size-12">
                                <Col sm={3} md={2} className="mb-2">
                                    <div className="color-gray-dark-v4">TANGGAL BUAT</div>
                                    <div>{d.order.summary.product[0].created}</div>
                                </Col>
                                <Col sm={3} md={2} className="mb-2">
                                    <div className="color-gray-dark-v4">TOTAL</div>
                                    <div>{d.order.summary.detail.total}</div>
                                </Col>
                                <Col sm={3} md={2} className="mb-2">
                                    <div className="color-gray-dark-v4">DIKIRIM KE</div>
                                    <div>{d.order.consumer.username}</div>
                                </Col>
                                <Col sm={3} md={4} className="ml-auto mb-2 text-sm-right">
                                    <div className="color-gray-dark-v4">ORDER #{d.order.summary.product[0].id}</div>
